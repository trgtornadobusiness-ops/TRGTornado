...
async function fetchStaticAlertsFallback(){
  ...
  for(const url of urls.slice(0,1)){
    try{const r=await fetch(url,{cache:"no-store"}); if(!r.ok) continue; const d=await r.json(); if(Array.isArray(d?.features)) return {features:d.features,meta:{staticFallback:true,checkedAt:d.updated||new Date().toISOString()}};}catch{}
  }
  return null;
}
...
function zoomTropical(delta,cx=null,cy=null){const viewport=$("#tropicalViewer"),image=$("#tropicalImage");if(!viewport||!image||image.style.display==="none")return;const old=tropicalViewer.scale,next=Math.max(1,Math.min(4,old*delta));if(next===old)return;if(cx!=null&&cy!=null){const r=viewport.getBoundingClientRect(),px=cx-r.left,py=cy-r.top;tropicalViewer.x=px-(px-tropicalViewer.x)*(next/old);tropicalViewer.y=py-(py-tropicalViewer.y)*(next/old);}tropicalViewer.scale=next;applyTropicalTransform();}
