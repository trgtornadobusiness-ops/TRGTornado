/* Complete active NWS alert center: no silent 5-alert cap, with pagination and accurate severe counts. */
(() => {
  ...
  const typeOf = event => /watch/i.test(event || "") ? "watch" : /warning|emergency/i.test(event || "") ? "warning" : "other";
  const score = event => {
  ...
  const isSevereWarning = p => /tornado warning|severe thunderstorm warning|extreme wind warning/i.test(p?.event || "");
  ...
  function render() {
    const filtered = sort(state.all.filter(x => state.filter === "all" || typeOf(x.properties?.event) === state.filter));
    ...
    box.innerHTML = visible.length ? visible.map((item, i) => {
      const p = item.properties || {}, event = p.event || "Weather Alert", type = typeOf(event);
    ...
    if (totalEl?.parentElement?.querySelector("span")) totalEl.parentElement.querySelector("span").textContent = `all active alerts • ${severe} severe warnings • ${tornado} tornado`;
