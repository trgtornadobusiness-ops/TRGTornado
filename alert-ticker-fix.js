...
(()=>{
  ...
  function render(features){
    ...
    drawing=true;
    if(!items.length){track.innerHTML='<span class="ticker-item ticker-clear"><span class="ticker-badge">NO TARGET ALERTS</span> No active target alerts.</span>';drawing=false;return;}
    const build=list=>list.map(f=>{const p=f.properties||{},k=kind(p.event),c=color[k],considerable=k==='severe'&&/considerable/i.test(`${p.headline||''} ${p.description||''} ${p.instruction||''}`),badge=considerable?'CONSIDERABLE':label[k],ms=Date.parse(p.expires||'')-Date.now(),mins=Number.isFinite(ms)?Math.max(1,Math.floor(ms/60000)):null,time=mins==null?'':mins<60?`${mins} MIN LEFT`:`${Math.floor(mins/60)}H ${String(mins%60).padStart(2,'0')}M LEFT`;return `<button type="button" class="ticker-item ${k}${considerable?' considerable':''}" data-alert-id="${esc(f.id||'')}" style="border-color:${c};color:${c}"><span class="ticker-badge" style="color:${c};border-color:${c}">${icon[k]} ${esc(badge)}</span><span class="ticker-area">${esc(area(p))}</span>${time?`<span class="ticker-time">${time}</span>`:''}</button>`}).join('<span class="ticker-separator">•</span>');
    const top=items.slice(0,12);track.innerHTML=build(top)+'<span class="ticker-separator ticker-loop-gap">•</span>'+build(top);requestAnimationFrame(()=>track.style.setProperty('--ticker-distance',`${Math.max(track.scrollWidth/2,900)}px`));drawing=false;
  }
