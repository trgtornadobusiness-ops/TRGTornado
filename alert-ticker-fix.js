/* TRG alert ticker patch v2: override the old app renderer and exclude generic Flood Warnings. */
(() => {
  ...
  function render(features){
    ...
    items.sort((a,b)=>rank[kind(b.properties?.event)]-rank[kind(a.properties?.event)]||Date.parse(a.properties?.expires||'')-Date.parse(b.properties?.expires||''));
  ...
  async function refresh(){try{const r=await fetch(`https://api.weather.gov/alerts/active?limit=5000&_trg_ticker_v2=${Date.now()}`,{cache:'no-store',headers:{Accept:'application/geo+json,application/json'}});if(!r.ok)throw new Error(`${r.status}`);const d=await r.json();render(d.features)}catch(e){console.warn('TRG ticker refresh failed',e)}}
  window.renderTicker=()=>render(Array.isArray(window.alertState?.national)?window.alertState.national:[]);
  refresh(); setInterval(refresh,60000); setInterval(()=>window.renderTicker(),5000);
})();
